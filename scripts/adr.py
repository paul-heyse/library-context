...
The architectural collection is current truth; an ADR is the why. `lint` checks that every
`design:` ref resolves to a stable section owner, that supersession links are
...
def render_index(adrs: list[Adr]) -> str:
    lines = [
        ...
        "`docs/design/DESIGN.md` plus `docs/design/sections/` owns architecture; "
        "these records are the why.",
        "",
        "## Active",
        "",

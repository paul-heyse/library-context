...
| L3 summaries | **Partially implemented; focused tests passed.** Direct, exact modeled, unique-assignment and unconditional local value paths with canonical ordered proofs; petgraph SCC scheduling and a bounded recursive value worklist (ADR-0053). Finite rules consume explicit inputs and publish typed refusals (ADR-0050); origin-specific summary/boundary IDs follow the unaggregated contribution (ADR-0054). Depth, unsupported predecessor and real 129-atom condition refusals survive native serving; a pure recursive pair cap has its own append-only reason. A narrow, source-ordered pinned normal-completion predecessor is cited before a direct return | General argument/predecessor completion, conditional recursive BDD/argument substitution, effect/exception/role summaries, a single-run two-origin Delta/native trace, remaining BDD work/node and recursive pair-cap Delta/native controls, `call_transfer` discharge |
...
| **W12 · [RF/F06](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F06)** | `lctx-analytics::summaries` | ADR-0053's deterministic SCC-local worklist propagates the first unconditional recursive value relation using cited callee paths, origin-specific proofs and depth/pair-work refusals. Pair exhaustion has append-only `summary_pair_work_limit` (code 25), distinct from general reach/BDD budget. Direct and independently proved modeled bases can seed a recursive component. Recompare pinned Ascent/datafrog when multiple recursive channels share rules | **Partial, targeted tests passed (2026-09-26):** pure self/mutual finite-base, no-base, parallel-origin, shuffle and low-cap controls; a real-source conditional self-call reaches the local seed without an unsupported positive. The codebook snapshot and synthetic native two-origin cap-reason admission pass. Source-level conditional finite-base admission still needs invocation-specific atom substitution and bounded BDD conjunction; Delta/native cap trace, other transfer channels, full product publication and pilot cost remain open | A real terminating recursive value path with exact argument/condition substitution and an independent withholding control; per-origin cap through `summary_boundaries` and native in one publication; shuffled published bytes; measured pilot delta |

...
**Status:** Active; the sole product execution plan. **Revised:** 2026-09-25 (pre-Stage-3
remediation checkpoint after targeted implementation; integrated acceptance deferred).
...
Production is on `main` through `7d02b42`; the pre-Stage-3 repairs after `acbcee5` include
...
| L7 serving | **Partially implemented.** FORMAT 7 structural condition/summary data; native `inspect_value_paths` gives path-local inspection only; operation facet values now retain verdicts in focused tests | Typed operation-wide compatibility/effect/role filters, full proof spans, W3 support closure and citations, clean-wheel query |
...
**Next dependency:** Finish W1's typed native projection decoder and a real finalizer-bearing
generation. Complete W4's published/served premise trace, then W3's support closure and W5's
explicit summary inputs and typed refusals. W7's SCC fixed point must publish a typed budget
boundary before dependent positives can treat missing sources as complete.
...
|---|---|---|---|---|
| <a id="ARC-01"></a>**W1 · ARC-01**, [RF/F13](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F13) | `cpg-schema` (`SummaryFlowStepKind`), `lctx_semantics` loader | The native loader now admits proof kinds from the schema codebook, including the finalizer kind, but still decodes positional string tuples. Centralize one typed projection decoder and choose transport by total conversion cost; IPC bytes alone do not make decoders agree | **Partial, targeted tests passed** (`8781246`): native proof-kind admission now follows the schema codebook; typed projection decoding and a real finalizer-bearing generation remain open | A1's acceptance; an unknown-kind rejection control; a generation/reader schema drift test |
...
| **W4 · [RFU/F05](../design_review/reviews/design_review_library-fit-reasoning-followup_2026-09-25.md#F05)**, [RF/F09](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F09) | `cpg-flow` / `cpg-core::flow_model` access boundary | Core flow now resolves bare, qualified and aliased builtin access; computed names become dynamic and shadowing is respected. Focused provider tests passed, but the corrected premise has not yet been traced through publication and serving | **Partial, targeted tests passed** (`5044fee`): resolved bare/qualified/aliased builtin access, computed names and shadowing; published/served premise trace remains open | Real-provider controls for all three spellings, computed names and shadowed builtins; a genuinely unread field stays distinguishable; one premise traced through publication and serving |
...
| **W6 · [RF/F01](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F01), [RF/F02](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F02), [RFU/F01](../design_review/reviews/design_review_library-fit-reasoning-followup_2026-09-25.md#F01)** | `cpg-schema::condition_kernel`, native generation preparation | F01 non-allocating `check_binary_op` decisions and F02 effective-support normalization are implemented; the native catalog now counts per-root retained closure as well as stored nodes (RFU/F01). The large-result, over-preflight contradiction, task-limit and aggregate diagnostic acceptance controls remain | **Partial, targeted tests passed** (`dd3bdcf`, `8781246`): non-allocating decisions, effective support and retained-node admission; large-result/task-limit/native aggregate acceptance controls remain open | F01: an admitted pair over 50k result nodes decides; an over-preflight contradiction decides; a task-limit control stays unknown. F02: redundant composition stays under the limit and live support equals hydrated support. RFU/F01: shared-tail and disjoint controls refuse before large allocation; native diagnostics distinguish stored from retained counts |
...
| **W8 · [RFU/F07](../design_review/reviews/design_review_library-fit-reasoning-followup_2026-09-25.md#F07)** | `cpg-extract` acquisition/context | The corpus identity now includes content and membership of every analyzer-readable root, including unselected helpers; ordinary site-packages were already hashed. A clean recomputation remains to qualify the changed-facts claim | **Partial, targeted test passed** (`b0d93a8`): all analyzer-readable corpus-root content and membership affect identity; clean recomputation/pilot pending | Editing and adding an unselected helper changes identity or is refused before extraction; relocation invariance holds; facts equal a clean recomputation |
...
| **W14 · [RF/F10](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F10), [RF/F11](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F11), [RF/F14](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F14), [RF/F15](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F15)** | `cpg-extract`, `cpg-flow`, `lctx-analytics` Pass B, `cpg-core` derivation | F10 resolved Pyrefly function flags/body kind are persisted and drive negative premises (schema migration); F11 ty settings come from run context and a virtual release root; F14 Pass B visited state includes suppression. F15 SQL-only tables still round-trip through Rust rows | **Focused implementation passed**: F10 (`4921a27`, schema migration), F11 (`5044fee`) and F14 (`df84905`). **Deferred**: F15 until the next edit of `attempt.rs` table construction; integrated qualification pending | F10: aliased-decorator and Protocol fixtures. F11: a provider operation actually affected by settings (e.g. resolution). F14: a fixture where the clean path is found second. F15: identical snapshots; independent rederivations kept |

...
| L3 summaries | **Partially implemented; focused tests passed.** Direct, exact modeled, unique-assignment and unconditional local value paths have canonical ordered proofs; a cited exact-boolean control now specializes one conditional recursive callee guard. Petgraph SCC scheduling and a bounded recursive value worklist implement ADR-0053. Finite rules consume explicit inputs and publish typed refusals (ADR-0050); origin-specific summary/boundary IDs follow the unaggregated contribution (ADR-0054). Depth, unsupported predecessor and real 129-atom condition refusals survive native serving; a pure recursive pair cap has its own append-only reason. A narrow, source-ordered pinned normal-completion predecessor is cited before a direct return | General argument/predecessor completion, general recursive BDD/argument substitution, effect/exception/role summaries, a single-run two-origin Delta/native trace, remaining BDD work/node and recursive pair-cap Delta/native controls, `call_transfer` discharge |
...
production-cap/native and cost controls still open. W5's real atom cap now survives publication
and native serving; work/node caps and a single-run two-origin Delta/native trace remain. ADR-0054
now gives contribution, summary and boundary rows stable origin IDs, and the native open-boundary
response exposes them; pure and native sibling controls passed separately. Continue order 1's
...
| <a id="ARC-03"></a>**W5 · ARC-03** | Summary producer, boundary publication | The producer owns typed admitted/refused outcomes; the same outcome publishes and validates boundaries. Append-only reasons distinguish depth, BDD work/node/atom limits; generic missing evidence does not erase `call_transfer`. [ADR-0054](../adr/0054-origin-specific-summary-identity.md) hashes each unaggregated contribution and carries it through modeled/direct/assignment/local seeds, summary proof identity, boundary key and native open response; the obsolete SQL-only complement is removed | **Partial, focused tests passed**: actual nine-call depth and unsupported predecessor refusals reach native. A real 129-atom return condition yields `condition_atom_limit` through Delta, FORMAT 8 and native response, with no positive summary. Actual predecessor conjunctions retain specific work/node-limit causes in the pure producer. The same-fact sibling-origin pure control and a native two-origin admission control pass separately (2026-09-26), as do reviewed schema snapshots and the real modeled-origin join fixture. Work/node-cap Delta/native publication and one real two-origin Delta/native generation remain open; no negative was emitted | An actual depth/BDD cap and a distinct unsupported case keep specific causes through producer, snapshot and native response; a two-origin real-source trace preserves both causes; neither becomes a negative; any codebook change is append-only |
...
| **W12 · [RF/F06](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F06)** | `lctx-analytics::summaries` | ADR-0053's deterministic SCC-local worklist propagates unconditional recursive value paths using cited callee summaries, origin-specific proofs and depth/pair-work refusals. One exact two-positional-argument case specializes a conditional callee truthy guard with a boolean literal, a direct formal-test link and bounded BDD restriction, retaining the caller root. Pair exhaustion has append-only `summary_pair_work_limit` (code 25); the new ordered `callee_condition_link` proof kind is code 14. Recompare pinned Ascent/datafrog when multiple recursive channels share rules | **Partial, targeted tests passed (2026-09-26):** pure self/mutual finite-base, no-base, parallel/open origin, shuffle and low-cap controls; the real terminating `recursive_base_identity(value, True)` source publishes a conditional depth-one proof while `conditional_self_recursive(value)` remains unknown. Codebook/rule snapshots, focused real Delta-to-native generation with the conditional proof, a missing-link native rejection and synthetic native cap-reason admission pass. General argument substitution/conjunction, other channels, Delta/native pair-cap trace and pilot cost remain open. [Scoped review](../design_review/reviews/design_review_recursive-literal-control_2026-09-26.md) | General condition/argument composition with BDD node/iteration budgets, a real opposing source control, per-origin cap through `summary_boundaries` and native in one publication; shuffled published bytes; measured pilot delta |

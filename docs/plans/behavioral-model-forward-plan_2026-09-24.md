...
| **W12 · [RF/F06](../design_review/reviews/design_review_library-fit-reasoning_2026-09-25.md#F06)** | `lctx-analytics::summaries` | ADR-0053 selects a deterministic SCC-local bounded worklist for the first recursive value channel after a pinned Ascent/datafrog/petgraph finite-base comparison. The comparison proves one relation, not full condition/proof/refusal composition; recompare libraries when multiple recursive channels share rules | **Interface-checked, partial (2026-09-26):** the isolated same-state probe agreed on finite-base reachability, shuffled input and a no-base control; a native cap remained unknown. ADR-0053 chooses ownership, but product recursive transfer, per-origin caps, native trace and pilot cost remain open | Order 6's fixtures under the chosen engine; shuffled-input identity; a cap cause in `summary_boundaries`; measured compile-time delta |

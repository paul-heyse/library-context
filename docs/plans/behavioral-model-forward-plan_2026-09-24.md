...
Production is on `main` through `d9d8a40`; the pre-Stage-3 repairs after `acbcee5` include
...
| L3 summaries | **Implemented; focused tests passed.** Direct, exact modeled, unique-assignment and unconditional acyclic wrapper value paths with canonical ordered proofs; petgraph SCC schedule; finite rules now consume explicit inputs and publish typed refusals (ADR-0050). A nine-call chain's depth refusal and an unsupported preceding-call control survive native serving | Normal-completion witnesses for compatible predecessors, recursive composition, effect/exception/role summaries, path-specific boundary identity, BDD-cap publication control, `call_transfer` discharge |
...
remains pending. Complete W5's BDD-cap publication control and coordinate path-specific identity
with Stage 3 order 3. W7's SCC fixed point must publish a typed budget

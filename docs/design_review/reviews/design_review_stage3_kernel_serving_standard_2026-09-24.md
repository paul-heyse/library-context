...
### Targeted design-gate addendum (2026-09-24)

**Scope and evidence.** This addendum inspects the revised ADR-0024/0025, DESIGN §§B13/3.9/11.3, the forward plan's Stage 3.0/3.6 gates, and the read-only [pilot conversion survey](../evidence/2026-09-24_bdd-pilot-survey/README.md). The survey is **Measured** for conversion of already-stated rows in snapshot `ecf8b9cdc11ebaf4c1dae61fa9b35dee`; the proposed compiler migration and served query path remain **Proposed**. No product build or suite was run for this addendum.

| Check | Evidence and interpretation | Design verdict |
|---|---|---|
| Construction before DNF loss | The survey converted all 13,775 stated conditions but could not reconstruct the one stored `over_budget` sentinel. That sentinel is referenced by 237 `flow_regions` and 846 `flow_reaching` rows. `cpg-flow/src/predicate.rs` and `lib.rs` currently compose `Condition` throughout translation, and `Condition::and`/`or`/`from_dnf` can return `OverBudget` before a row is written. Moving only the final stored-row conversion would therefore preserve source loss. ADR-0024's chosen BDD design must compose at those producer sites before the first lossy DNF operation, and `cpg-core::flow_model`, which parses stored encodings, must migrate with the root/node schema. The evidence requires an upstream lossless representation; it does not establish BDD as the only possible representation. | **Pass at design level** for ADR-0024 and the plan's sequencing; G2/G6 and CI-G1/G2 remain product gates. |
| Claimed benefit | The 0.97 s survey duration covers conversion/validation after fetching existing rows, not a new compiler pass, persisted node publication or serving. The 23,760 unique nodes estimate graph size, not Delta/bundle size. The survey cannot attribute all 66 recorded `budget_reached` claims to recoverable functions or predict their reduction. ADR-0024 and the plan now explicitly require a product pilot to measure whether they fall. | **Pass with the corrected claim**; Stage 3.0 pilot and limits remain **not_run** for the migration. |
| Native package gate | The foundation checkpoint already **Tested** an sdist, wheel and isolated CPython 3.14.7 import of developer smoke probes (compact review). Editable `uv run` plus focused tests is a suitable design loop, but cannot verify the wheel's compiled module, dependency closure or installed server route after the native API and generation loader change. ADR-0025, DESIGN §B13 and the plan now require a clean installed-wheel, generation-pinned FastMCP/native round trip at the Stage 3.6 serving acceptance checkpoint and again at release. | **Pass at design level** for G7 timing; the installed product query remains **not_run**. No wheel is required on every design iteration. |

**Disposition.** The corrected before-DNF migration rule is supported and the current DESIGN §3.9 appropriately distinguishes implemented Stage 2 DNF from the proposed BDD authority. The accepted-proposal decision above remains valid. The revised Stage 3.6 wheel gate closes the design timing gap; product acceptance still requires its execution against a pinned generation. No exception is recorded. Repeating the wheel check at release is deferred until the distributable API and format settle.

### Leaf-atom proof join addendum (2026-09-24)

**Targeted check.** Proposed `flow_test_types` and `flow_test_value_links` now name a leaf evaluation atom as well as a test/use/span. This is the right variable key for a compound test: Stage 2's `Atom::Evaluated` encodes the semantic atom with `Site(module-key, byte range)` or a `Synthetic(module-key, provider-predicate-id)`. `Translator::test` gives distinct Boolean operands their own sites; `isinstance(x, (A, B))` can give two same-site atoms distinct class-bearing encodings. The leaf key therefore preserves, rather than contradicts, Stage 2 identity. This is **Inspected** source/design evidence, not a product proof-relation test.

| Finding | Evidence and consequence | Required correction and gate |
|---|---|
| **F05 — not every synthetic leaf has a retained `flow_tests` row** | `cpg-flow/src/lib.rs` collects ty predicates but keeps only the first `Test` per `Span` (`seen.insert(span)`). Every `PredicateNode::Pattern` uses its `match` subject span, while `synthetic_identity` includes the distinct scoped predicate id. The existing `matched(command)` fixture has `case "start"` and `case None`; its pinned flow snapshot contains separate `equals(command,"start")#…:p…` and `is_none(command)#…:p…` atoms in region conditions. A later pattern atom can thus be in the condition graph while absent from the one retained `flow_tests` condition at that subject span. ADR-0024's proposed validator, which requires the atom to be in that test condition's support, cannot admit an otherwise valid leaf proof. Silently omitting the proof is sound but loses the promised join coverage. | Before producing `flow_test_types`, retain each predicate/leaf test identity or create an authoritative leaf-atom relation emitted ahead of span-only deduplication. Let both proof relations cite that row, verify the atom and its operand use against its own condition/source identity, and leave synthetic leaves without a justified operand mapping unknown. Add a two-arm `match` fixture and a compound Boolean fixture. **G2/G6/CI-G1/CI-G2: Revise at design level until the proof-row authority is specified; product gate open.** |

The Stage 3.0 implementation order correctly moves lossless Boolean construction ahead of persistence. F05 concerns proof attribution after that migration; it does not change the BDD construction or wheel-gate conclusions above.

**Corrected-proposal disposition (2026-09-24).** ADR-0024 now makes `flow_test_leaves` the proof authority: `cpg-flow` emits a row for each provider predicate and leaf before the span-only `flow_tests` projection, recording scope, predicate identity, test/leaf spans, condition root and encoded evaluation atom. `flow_test_types` cites its fact id and an exact `flow_uses` operand; publication checks that the atom belongs to that leaf row's own root support and that the same-snapshot use, span, type origin and cited facts match. ADR-0025 makes `flow_test_value_links` cite the same leaf fact id and requires a separate subject-use proof for synthetic patterns. DESIGN and Stage 3.0's order carry both contracts and name compound Boolean and two-arm `match` fixtures. This removes the span-deduplication counterexample at the **Proposed** design level: **F05 corrected; G2/G6/CI-G1/CI-G2 pass at design level**. The new relation, producer, validators and fixtures remain **not_run** product gates. No further precise design gap was found in this targeted check; an absent or unjustified leaf-to-use mapping must remain unknown.

**Row-contract consistency check (2026-09-24).** The proposed Arrow key `(snapshot_id, module_node_id, predicate_key, atom_id)` retains separate same-span pattern predicates. ADR-0024 now defines `predicate_key` with the exact `flow-synthetic-predicate` hash inputs used by Stage 2; the synthetic atom's separate module key is recomputed from the cited source path/text. `atom_id` hashes the full `Atom::encode()` while BDD nodes retain that full encoding, so root-support membership can be checked without a library-local variable index. The `Site` span must equal its encoded site, and a synthetic leaf's subject span is not by itself a proof of its tested operand. `FlowUsesRow` has a stable `use_id` from module node and byte range; the read-only pilot probe reports zero duplicates, but ADR-0024 correctly requires **exactly one** same-snapshot/module row with the matching operand span and decoded `tested_place` before a proof is admitted. These contracts are **Proposed** and source-consistent; the probe does not test their new producer or validator. **F05 remains corrected at design level; no additional finding.**

**Entry-value bridge probe check (2026-09-24).** I reran `target/release/lctx flow docs/design_review/evidence/2026-09-24_entry-value-bridge/probe.py` (**passed**) and checked the [probe report](../evidence/2026-09-24_entry-value-bridge/README.md) against its JSON. `direct` has one formal reaching the tested `mode` use; `rebound` reaches its assignment; `changed_closure` includes a null/unknown source beside the formal; `after_call` still reports the formal at the second guard despite an intervening callback. These are **Tested** Stage 2 flow observations, not tests of the proposed link. In particular, the callback probe does not show that `mode` changed at runtime; it shows that static reaching alone cannot justify stable equality meaning or an entry-value-to-guard proof across an unmodeled call. ADR-0025 and the plan now require exact leaf/operand attribution, unique formal reach and a checked no-effect path for `direct_parameter_reach_no_effect`, with later call crossing tied to a cited identity-preserving summary and effect-model revision. **F01's proposal-level correction remains sound; G2/CI-G1 pass at design level.** The producer, validator, runtime counterexamples and generation-pinned query remain **not_run** product gates. No new design finding arises from this probe.

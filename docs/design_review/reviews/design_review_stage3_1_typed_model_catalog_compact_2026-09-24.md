...
dependency targets require a matching distribution/version and site-package module. External
definitions are sparse, so a model without a referenced callable row remains dormant even when
its module is present. A bound model with an unresolved formal fails before writes.
...
model, per bound callable/overload. `context_definitions.signature_count` and raw
`context_parameters` come from the same pinned Pysa collector used for release signatures;
each authored formal must appear exactly once in every complete signature. The shared
validator reconstructs both model tables from the
...
| CI-G1 / CI-G2 / CI-G3 | Pass for attributed model rows; unresolved for serving | Every transfer's formal has pinned signature witnesses, but summaries and a generation-pinned native consumer are absent. |
...
|---|---|---|
| Resolved M01 (2026-09-24) | `Parameter[val]` needed a pinned formal witness. | `context_parameters` now records Pysa's signature forms and names for referenced external functions; the compiler requires the formal in every complete signature before materializing a transfer. The real `typing.cast` fixture publishes positive rows. |
...
| Deferred M04 | The focused `typing.cast` fixture publishes positive model rows, but no positive model row has yet been checked on the pilot under the final catalog; native serving has no model reader. | Confirm binding counts and exact model revisions in the final fresh-store pilot, then require the one-generation native load check. |

## Verification

**Passed, 2026-09-24:** focused release-profile Nextest selections cover strict parsing,
pin mismatch, dormant unreferenced model, compiled paths, schema/codebook/rule snapshots, tampered
target/transfer rows, the synthesis ledger, the real `typing.cast` formal fixture and
no-analysis publication. Focused Clippy on `cpg-schema`, `cpg-extract` and `cpg-core`
passed with `-D warnings`; `cargo check --workspace` passed.

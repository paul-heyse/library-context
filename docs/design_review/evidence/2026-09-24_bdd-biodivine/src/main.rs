...
fn main() {
    ...
    let inner = vars2
        .mk_literal(v2[0], true)
        .and(&vars2.mk_literal(v2[1], false));
    let merged = vars2
        .mk_literal(v2[0], true)
        .and(&vars2.mk_literal(v2[0], false));
    println!(
        "two sites satisfiable={} one atom satisfiable={}",
        !inner.is_false(),
        !merged.is_false()
    );
    ...
    println!(
        "F&N == c&N (factor out N): {}",
        f.and(&n) == vars.mk_literal(v[c], true).and(&n)
    );
    println!(
        "compatible(F, !a & !b) = {}",
        !f.and(&vars.mk_literal(v[a], false))
            .and(&vars.mk_literal(v[b], false))
            .is_false()
    );
    // Size limit.
    ...
    for (nvars, terms, lits) in [(20usize, 16usize, 8usize), (40, 16, 8)] {
        ...
        for w in conds.windows(2) {
            match Bdd::binary_op_with_limit(
                50_000,
                &w[0],
                &w[1],
        ...
        let conj = t1.elapsed();
        let t2 = Instant::now();
        let rendered: usize = conds
            .iter()
            .take(50)
            .map(|c| c.to_optimized_dnf().len())
            .max()
            .unwrap_or(0);
        println!(
    ...
    println!(
        ...
        format!("{:?}", again.to_optimized_dnf()) == format!("{:?}", x.to_optimized_dnf())
    );
}

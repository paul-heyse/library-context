...
| Gold-reference paths (`.claude/skills/**`) read by the compiler, or analytics parameters tuned against the gold | Hidden input; circular evaluation | DM-28, DM-59 · G4 | `ast-grep` rule on skill paths in compiler code; test that no library definition (`libraries/*/pyproject.toml`) or environment path points into `.claude/skills/` |

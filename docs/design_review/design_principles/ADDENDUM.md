...
| §2 extracted, resolved, derived and heuristic stay distinguishable; unknown targets explicit | a derivation class per edge kind, published in `edge_kinds`; `facts` provenance; candidate sets kept; external and synthetic nodes; unresolved remainders and provider reasons, no catch-all; `graph_gaps` for rows not yet in the graph | §3.2, §3.5, §3.6, §3.7, §3.8 | lineage rules; `typed:*` rules and their raw-mutation tests; the `pysa_calls` partition rules; `edge_kinds` in the derivations snapshot |
| §3 a projection's spec (*consumer*); separate output selector and universe; declared direction; weight semantics | the CPG supplies each edge kind's direction meaning, parallel policy, evidence and completeness; the spec itself lands with §5 | §3.7, §3.8, §5 | the first projection's review |
| §4 graph-local indices never become identities; simplification preserves the question (*consumer*) | §5's three identities; the catalogs hold no indices | §3.8, §5 | the graph-readiness reader; the first projection's tests |
| §7 no eager closures or path enumeration; partial is not complete | the catalogs hold direct relations only; coverage, boundaries, external and synthetic nodes and `graph_gaps` state where the graph stops | §3.7, §3.8 | review question 13; the partition rules |
| §10 results carry run and projection lineage and evidence (*consumer*) | `findings`, `witnesses`, `evidence` (§3.2) | §3.2, §9 | the §8 assertion rules |
| §11 Delta read through a pinned snapshot; a manifest after validation; retention for files live snapshots need | `snapshots` append; pinned `with_version` reads with a `snapshot_id` filter; cleanup off, retention verified at open | §6.1, §6.2 | the reader tests; the retention test |
| §12 known-answer shapes (isolates, parallel edges, self-loops, reconvergence, cross-file cycles, unresolved, mixed configuration); lineage; per-stage instrumentation | the `graph_shapes`, `syntax_shapes`, `lexical_shapes` and `type_shapes` fixtures; generated lineage rules; `lctx compile` stage metrics | §3.8, §4.3 | insta catalog snapshots; `just pilot` |

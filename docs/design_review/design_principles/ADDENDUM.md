...
| §2 extracted, resolved, derived and heuristic stay distinguishable; unknown targets explicit | a derivation class per edge kind; `facts` provenance; candidate sets kept; external and synthetic nodes; unresolved remainders and reasons | §3.2, §3.5, §3.6, §3.7, §3.8 | lineage rules; `semantic:call-target-typed`; the codebook snapshot |
...
| §7 no eager closures or path enumeration; partial is not complete | the catalogs hold direct relations only; coverage and boundaries state where the graph stops | §3.7, §3.8 | review question 13 |

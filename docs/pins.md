...
| Component | Pin | Verified | How |
|---|---|---|---|
| pyrefly (library) | git `github.com/paul-heyse/pyrefly` rev `b9f28575ce2baa93dbc416670a34592501b3fcd4`, in-process. That is tag 1.3.1 (`3e3177d0f4755b56c2d5a710d830eed89b14c2e3`) plus `third_party/pyrefly-1.3.1.patch` (sha256 `b7b82828e3e3470c93d35d5aef0e24904cd3b570b61772f4883701805959b767`; 6 files, +22/−11: visibility, a `write_files` switch, a `pysa_reporter()` borrow). The fork was published 2026-09-22 as branch `lctx/1.3.1` | 2026-09-22 | `git ls-remote https://github.com/paul-heyse/pyrefly.git refs/heads/lctx/1.3.1` gives `b9f28575`. `git format-patch -1` on a fresh clone of the branch matches the patch sha256. Spike (`dcdc969`) built by `git`/`rev` on 1.98.1 stable with the §7 family: `check_family.py` extended to ruff, pyrefly and blake3 (one version each), and `cargo deny check bans sources licenses` passed; FastMCP output byte-identical to the local-clone runs |

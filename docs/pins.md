...
## Analytics crates (ADR-0011; leiden-rs a dependency from slice 2.3, fcars planned)
...
| httpx | `==0.28.1` (the query embedder; bytes sent as `content=`, never `json=`) | 2026-09-23 | `uv lock`; `test_the_http_client_sends_the_exact_bytes_and_reports_a_down_service` |

...
- **One producer per table.** A fact table is written by one producer: one extractor surface, or
  ...
  **once per extractor run**, which carries Stage A's output; later producers reference
  ...
  distinct releases (the library and its corpus), since the family keys carry no run; the corpus
  run shares the library's context, so `distributions` is written once. `distributions` is keyed by `context_id`:
...
terms with two binders, the recursive alias as one finite term). C5a is **Implemented** and
**Tested** (`a_corpus_documents_its_library`: the selection, passages, a code block inside an MDX
component, links, mentions by class, an unparsable document's coverage); C5b is **Proposed**. This
is how
...
| C5 (C5a **Implemented**) | `contains_passage`: document → passage, ordinal; `contains_block`: passage → code block, ordinal (extracted). `mentions`: passage → export, class or function, at the byte offset (recognizer; parallel; `exact` or `lexical` and the modality on the evidence row). C5b: `block_module` (code block → its materialized module), `usage_link` (usage → release declaration; joined) |
...
- **Docs (C5):** `id:documents`, `id:passages`, `id:code_blocks`; `typed:mention_targets`; a
  run's release has modules or documents; `coverage:complete` expects a `docs` row per document.

...
- **Important controls:** one `parameter` assertion per parameter of the seed's own signature (the
  ...
  - Its description, when the documentation gives one (`documented`): the docstring's
    ...
    paragraph's bytes as Passage evidence. If several such fields disagree after normalization,
    none is used. A `<ParamField>` nested in another describes a field of its parent's value,
    never a parameter.
...
- **Documented warnings** (Limits; slice 3.4, rebuilt on components by A3, Implemented and
  ...
  read. Warnings enter the brief document as the capability's own limits.
- **Measured on the pilot** (2026-09-24, snapshot `8c76bc75…`, 20 briefs): the corpus has 1,250
  ...
  that is no exact mention. A code-block call anchor would reach 13 warnings in 7 briefs and no
  `<ParamField>` (that block's `mcp` is unbound), and it is noisy (`TransportMixin.run` anchors 44
  passages through `mcp.run()`). The anchor is an open content question (deviation log D45),
  settled by Phase 4's structured evaluation of the holistic assessment's plan.
